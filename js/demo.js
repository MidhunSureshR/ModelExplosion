...
import Sketch from "./icosahedron"
{
    ...
    let animation = new Sketch(
        ...
        {
            surface: 'e8e8e8',
            inside: '39FF14', //Fluorescent green
            background: '151616',
            onLoad: ()=>{
                document.body.classList.remove('loading');
                //Set initial position
                animation.scene.rotation.x = 1.57;
                animation.scene.rotation.y = animation.scene.rotation.z = 0.0348;
            }
        },
        true
    );
    ...
    let elem = document.getElementById("posIndicator");
    const draw = () => {
        ...
        if ( animation ) {
            ...
            animation.scene.rotation.y = Math.PI / 2 - Math.PI - ta*(2 - ta)*Math.PI * sign(mouseX);
            animation.scene.rotation.z = Math.PI - ta*(2 - ta)*Math.PI * sign(mouseX);
            elem.innerText = "Rotation x = " + animation.scene.rotation.x + " y = " + animation.scene.rotation.y + " z = " + animation.scene.rotation.z;
        }
